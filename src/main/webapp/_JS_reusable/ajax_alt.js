...
* - If the call was successful, invoke the consumer's successFn (passing back 
*   a JS object that was converted from the JSON onf the page).
* - If the call was unsuccessful, invoke the consumer's failureFn (passing back 
*   a good error message. 
*/
console.log("ajax_alt has been created");
function ajax_alt (url, successFn, failureFn) {

    console.log("ajax_alt has been called with URL "+url);
    ...
    // The httpReq Object is now local to function "ajaxCall" (not global)
        ...
        function callBack() {
            console.log("callBack in ajax_alt, httpReq.readyState is  " + httpReq.readyState);
